...
class DocxParseService:
    ...
    def extract_blocks(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        for item in elements:
            ...
            if isinstance(element, Paragraph):
                ...
                # ۱. اعتبارسنجی اولیه: آیا متن خصوصیات بدنه (طولانی، علائم پایان جمله) را دارد؟
                if self._looks_like_body(normalized_text):
                    # اگر در ورد استایل عنوان داشته ولی متن بدنه است، تغییر نوع داده می‌شود
                    heading_level = None
                    block_type = "paragraph"
    ...
    def _fix_half_spaces(self, text: str) -> str:
        ...
        text = re.sub(
            rf"(?<![{PERSIAN_LETTERS}])(ن?می)\s+([{PERSIAN_LETTERS}])",
            rf"\1{ZWNJ}\2",
            text,
        )
        text = re.sub(
            rf"([{PERSIAN_LETTERS}])\s+(ها|های|هایی|تر|ترین|ای)(?![{PERSIAN_LETTERS}])",
            rf"\1{ZWNJ}\2",
            text,
        )
        return text
    ...
    def _resolve_run_font_size(self, run: Run, paragraph: Paragraph) -> float | None:
        """
        استخراج اندازه فونت با پشتیبانی از ارث‌بری استایل‌ها در Word
        """
    ...
    def _paragraph_max_size(self, paragraph: Paragraph, default_size: float = 12.0) -> float:
        """
        محاسبه اندازه فونت مؤثر پاراگراف با جستجو در سطح Runها و ارث‌بری استایل
        """
        ...
        # اگر هیچ Run خاصی فونت نداشت، بررسی مستقیم استایل پاراگراف
        style = paragraph.style
        while style:
            if hasattr(style, "font") and style.font and style.font.size:
                return style.font.size.pt
    ...
    def _looks_like_body(self, text: str) -> bool:
        """
        بررسی اینکه آیا متن، خصوصیات بدنه (طولانی بودن، داشتن چند جمله) را دارد یا خیر.
        """
    ...
    def _format_metadata(self, element: Any, body_size: float = 12.0, was_reclassified: bool = False) -> Dict[str, Any]:
        ...
        # استخراج تنظیمات صفحه‌بندی (Pagination & Layout)
        ...
        # محاسبه اندازه فونت کشف‌شده بر اساس ارث‌بری
