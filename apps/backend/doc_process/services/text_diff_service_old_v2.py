...
# from django.conf import settings
# from django.db import transaction
# from doc_process.models import Document, DocumentBlock, ReviewJob

# logger = logging.getLogger(__name__)

# class LlmClientService:
#     """
#     Service to interact with the external FastAPI LLM review engine.
#     """
    
#     API_URL = "http://169.171.234.4:8000/api/v1/review"
#     DEFAULT_TIMEOUT = 120  # Timeout in seconds (long timeout for LLM processing)
...
#     ) -> dict:
#         """
#         Prepares the exact JSON payload required by the FastAPI review service.
#         """
#         # Fetching blocks ordered by order_index to maintain document flow
#         blocks_qs = DocumentBlock.objects.filter(document=document).order_index()
#         blocks_payload = []

#         for block in blocks_qs:
#             # Finding the parent heading if it exists
#             parent_heading_data = None
#             if block.parent_heading:
#                 parent_heading_data = {
#                     "title": block.parent_heading.raw_text or "",
#                     "level": block.parent_heading.heading_level or 1
#                 }
...
#                 "parent_heading": parent_heading_data
#             })
...
#     @classmethod
#     def send_to_review_engine(cls, payload: dict) -> dict:
#         """
#         Sends the payload to the external service and returns the response.
#         """
#         headers = {
...
#         logger.info(
#             f"Sending review request for Document {payload['document_id']} "
#             f"and Job {payload['review_job_id']} to API."
#         )
...
#             logger.info(f"Successfully received response from LLM service for Document {payload['document_id']}.")
#             return response.json()
            
#         except requests.exceptions.Timeout as e:
#             logger.error(f"Timeout connecting to LLM service: {str(e)}")
#             raise RuntimeError("The LLM review service timed out.") from e
#         except requests.exceptions.RequestException as e:
#             logger.error(f"Failed to connect or received error from LLM service: {str(e)}")
